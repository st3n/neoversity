...
from db.finish.dto import MoviesDTO, MovieDTO
...
class MovieCSVParser(MovieParserInterface):
    ...
    def _extract_unique_values(self, df: pd.DataFrame) -> Tuple[Set, Set, Set, Set]:
        ...
        for _, row in df.iterrows():
            movie_genres = {
                row_genre.strip() for row_genre in ast.literal_eval(row["Genre"])
            }
            genres.update(movie_genres)

            movie_directors = {
                row_director.strip()
                for row_director in ast.literal_eval(row["Director"])
            }
            directors.update(movie_directors)

            movie_stars = {
                row_star.strip() for row_star in ast.literal_eval(row["Stars"])
            }
    ...
    def _create_movie_dto(self, row: pd.Series) -> MovieDTO:
        ...
        if isinstance(certification, str) and certification.strip():
            ...
            movie_certification = "Not Rated"
        movie_directors = {
            row_director.strip() for row_director in ast.literal_eval(row["Director"])
        }
        movie_stars = {row_star.strip() for row_star in ast.literal_eval(row["Stars"])}
        description = " ".join(
            [desc.strip() for desc in ast.literal_eval(row["Description"])]
        )
...
if __name__ == "__main__":
    ...
    parser = MovieCSVParser(csv_path)
